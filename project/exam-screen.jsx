...
function Option({ idx, text, q, selected, revealed, onClick }) {
  ...
  let mark = CIRC[idx];
  if (revealed) {
    if (inAns) {cls += ' correct';mark = <Icon.check style={{ width: 15, height: 15 }} />;} else
    if (selected) {cls += ' wrong';mark = <Icon.x style={{ width: 15, height: 15 }} />;}
  } else if (selected) {
    cls += ' selected';
  }
    ...
    <div className="exam">
      <div className="exam-top">
        <div className="wrap exam-top-inner">
          ...
          (!showTimer &&
          <div className="timer mono"><Icon.check style={{ width: 14, height: 14 }} />{answeredCount}/{total}</div>)}
